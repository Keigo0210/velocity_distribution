...
def configure(config_file: Path):
    global CONFIG_FILE, CONFIG, OUTPUT_DIR, CASE_FILE, TIME_POINT, VELOCITY_NAME
    global COORDINATE_SCALE, COORDINATE_UNIT, VELOCITY_SCALE, VELOCITY_UNIT
    global GEOMETRY_FILE, FLIP_S_AXIS, FLIP_T_AXIS, SECTIONS_FILE, PLOT_TITLE_SUFFIX
    CONFIG_FILE = config_file.resolve()
    CONFIG = load_config(CONFIG_FILE)
    OUTPUT_DIR = resolve_config_path(CONFIG.get("output_dir", "output/slice_velocity_star_ccm"))
    CASE_FILE = resolve_config_path(CONFIG.get("case_file"))
    TIME_POINT = int(CONFIG.get("time_point", 299))
    VELOCITY_NAME = CONFIG.get("velocity_name", "Velocity")
    COORDINATE_SCALE = float(CONFIG.get("coordinate_scale", 1000.0))
    COORDINATE_UNIT = CONFIG.get("coordinate_unit", "mm")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Export STAR-CCM+ velocity on configured sections.")
    ...
    print(f"Input data: {CASE_FILE}; time point={TIME_POINT}")
    print(f"Output root: {output_root}")

    mesh, target_file = load_latest_mesh()
    reference_geometry, geometry_label = load_reference_geometry(mesh)
    bounds = mesh.bounds
    print(f"Mesh bounds ({COORDINATE_UNIT}): x={bounds[0]}..{bounds[1]}, y={bounds[2]}..{bounds[3]}, z={bounds[4]}..{bounds[5]}")
